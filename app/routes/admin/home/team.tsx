...
const Team = () => {
    ...
    const submit = async (id: number) => {
        ...
        if (nameRef.current?.value == null || nameRef.current?.value == undefined || nameRef.current?.value == "") {
        ...
        else if (dobRef.current?.value == null || dobRef.current?.value == undefined || dobRef.current?.value == "") {
            setError("Enter the date.");
        }
        else if (numberRef.current?.value == null || numberRef.current?.value == undefined || numberRef.current?.value == "") {
            setError("Enter the number.");
        }
        else if (positonRef.current?.value == null || positonRef.current?.value == undefined || positonRef.current?.value == "") {
            ...
            let sendata: any = { "name": nameRef.current?.value, "number": numberRef.current?.value, "positon": positonRef.current?.value, "dob": dobRef.current?.value, "description": descriptionRef.current?.value, "id": id };
            if (img == null) {
    ...
    const submitNew = async () => {
        if (nameNewRef.current?.value == null || nameNewRef.current?.value == undefined || nameNewRef.current?.value == "") {
            setError("Enter the name.");
        } else if (dobNewRef.current?.value == null || dobNewRef.current?.value == undefined || dobNewRef.current?.value == "") {
            setError("Enter the date.");
        }
        else if (numberNewRef.current?.value == null || numberNewRef.current?.value == undefined || numberNewRef.current?.value == "") {
            setError("Enter the number.");
        }
        else if (positonNewRef.current?.value == null || positonNewRef.current?.value == undefined || positonNewRef.current?.value == "") {
            ...
            const imageurl = await UploadFile(img);
            let sendata: any = { "name": nameNewRef.current?.value, "number": numberNewRef.current?.value, "positon": positonNewRef.current?.value, "dob": dobNewRef.current?.value, "description": descriptionNewRef.current?.value, "imageUrl": imageurl };
            const res = await axios({
                method: 'post',
                ...
                <div className="bg-[#31353f] rounded-lg p-4">
                    <div className="mx-auto">
                        ...
                        <div className="text-white grid place-items-center">
                            ...
                            <p>{new Date(viewDate.dob).toLocaleDateString('en-US', {
                                weekday: 'long',
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
                ...
                <div className="bg-[#31353f] rounded-lg p-4 w-80">
                    <div className="mx-auto">
                        ...
                        <div className="text-white">
                            ...
                            <input ref={numberRef} type={"number"} name="number" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the number.." autoComplete="off" value={editDate.number} onChange={(e) => setEditData((data: any) => ({ ...data, number: e.target.value }))} />
                            <input ref={positonRef} type={"text"} name="positon" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the position.." autoComplete="off" value={editDate.positon} onChange={(e) => setEditData((data: any) => ({ ...data, positon: e.target.value }))} />
                            <input ref={dobRef} type={"date"} name="dob" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the dob.." autoComplete="off" value={editDate.dob.substring(0, 10)} onChange={(e) => setEditData((data: any) => ({ ...data, dob: e.target.value }))} />
                            <textarea ref={descriptionRef} name="description" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the description.." autoComplete="off" value={editDate.description} onChange={(e) => setEditData((data: any) => ({ ...data, description: e.target.value }))} ></textarea>
                        </div>
                ...
                <div className="bg-[#31353f] rounded-lg p-4 w-80">
                    <div className="mx-auto">
                        ...
                        <div className="text-white">
                            ...
                            <input ref={numberNewRef} type={"number"} name="number" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the number.." autoComplete="off" />
                            <input ref={positonNewRef} type={"text"} name="positon" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the position.." autoComplete="off" />
                            <input ref={dobNewRef} type={"date"} name="dob" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the dob.." autoComplete="off" />
                            <textarea ref={descriptionNewRef} name="description" className="py-1 px-2 rounded-md border border-white w-full my-2 bg-transparent outline-none focus:bg-transparent fill-none" placeholder="Enter the description.." autoComplete="off" ></textarea>
                        </div>
                        {(error == "" || error == null || error == undefined) ? null :
                            <div className="bg-red-500 bg-opacity-10 border-2 text-center border-red-500 rounded-md text-red-500 text-md font-normal text-md my-4">{error}</div>
                        }
