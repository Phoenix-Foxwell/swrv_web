...
const User = () => {
    ...
    return (
        <>
            {/* view box */}
            <div className={`fixed h-full w-full bg-slate-800 bg-opacity-40 top-0 left-0 place-items-center py-20 overflow-x-scroll ${viewBox ? "grid" : "hidden"}`}>
                <div className="bg-[#31353f] rounded-lg p-4 w-80">
                    <div className="w-full grid place-items-center mb-4">
                        <img src={viewDate.pic == undefined || viewDate.pic == undefined || viewDate.pic == "" || viewDate.pic == "0" ? "/images/avatar/user.png" : viewDate.pic} alt="avatar" className="w-40 h-40 object-cover object-center rounded-md" />
                    </div>
                    <div className="mx-auto">
                        <div className="text-white">
                            ...
                            <p>Date of Birth : {new Date(viewDate.dob).toLocaleDateString('en-US', {
                                weekday: 'long',
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
            ...
            <div className={`fixed h-full w-full bg-slate-800 bg-opacity-40 top-0 left-0 place-items-center ${delBox ? "grid" : "hidden"}`}>
                <div className="bg-[#31353f] rounded-lg p-4 w-80">
                    <div className="mx-auto">
                        <div className="text-white">
                            <p>Are you Sure you want to change the status of {delDate.userName} user?</p>
                        </div>
                        <div className="w-full h-[1px] bg-gray-300 my-2"></div>
                        <div className="flex w-full justify-between">
                            ...
                            <div onClick={() => statusButton(delDate.id, delDate.status.code)} className={`text-sm flex gap-2 items-center my-1 b  py-1 px-2 rounded-md border border-gray-300hover:border hover:border-green-400 hover:bg-green-500 hover:bg-opacity-10 hover:text-green-500 text-gray-300 cursor-pointer`}>
                        ...
                        user.map((val: any, index: number) => {
                            return (
                                <div key={index} className="bg-[#31353f] hover:bg-slate-800 rounded-md flex px-4 py-2 my-2 text-white font-medium text-md flex-nowrap">
                                    ...
                                    <div className="grow"></div>
                                    <div className="w-24">
                                        {Number(val.userStatus) ? <div className="w-16 py-1 text-white text-xs bg-green-500 text-center rounded-md font-medium">ACTIVE</div> : <div className="w-16 py-1 text-white text-xs bg-rose-500 text-center rounded-md font-medium">INACTIVE</div>}
                                    </div>
                                    <div className="grow"></div>
