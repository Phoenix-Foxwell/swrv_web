...
var UserInsights = (props) => {
  ...
  const init = async () => {
    ...
    const userdatareq = await axios_default.post(`${BaseUrl}/api/get-insta-handel-byid`, {
      userId: props.userId,
      handleId: handle_id
    });
    setPlatformData((val) => userdatareq.data.data[0]);
  };
  (0, import_react2.useEffect)(() => {
    init();
  }, []);
  const getPlatform = async (handle_id2) => {
  ...
  const handeladdupdate = async (handle_id2) => {
...
//# sourceMappingURL=/build/routes/home/myuser.$id-5UXAELJX.js.map
